...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1_d + opt.lambda_dssim * (1.0 - ssim(depth, gt_depth))
        loss.backward()
        ...
        if iteration <= opt.iterations and iteration % 100 == 0:           
            ...
            imageio.imwrite(os.path.join(scene.model_path, f"tmp_debug_depth", f"{iteration:05d}.png"), to8b(cm.inferno(rendered_depth)))
            imageio.imwrite(os.path.join(scene.model_path, f"tmp_debug_depth_gt", f"{iteration:05d}.png"), to8b(cm.inferno(normalize_im_gt(gt_depth_, gt_depths))))
